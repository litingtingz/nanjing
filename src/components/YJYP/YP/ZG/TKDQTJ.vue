          ...
          <div v-if="falg">
            <el-table
               ...
               <el-table-column
                 ...
                 <el-button type="text"  class="a-btn"  title="编辑"  icon="el-icon-edit-outline" @click="$router.push({name:'TKDQRYXX',query:{row:scope.row,queryPd:pd}})"></el-button>
                 </template>
               </el-table-column>
            </el-table>
       ...
       <el-dialog title="详情" :visible.sync="detailsqDialogVisible"  custom-class="big_dialog combine" :append-to-body="false" :modal="false">
         <el-form  ref="detailForm">
           <el-row :gutter="1"  class="mb-6">
               ...
               <el-col :span="8" class="input-item">
                <span class="input-text">办证类别：</span>
                <span class="input-input detailinput">{{detailForm.SQLB_DESC}}</span>
               </el-col>
               <el-col :span="8" class="input-item">
                <span class="input-text">户籍地：</span>
      ...
      methods: {
        ...
        getList(currentPage, showCount, pd,type) {
          ...
          if(this.checkedList.length==0){
            ...
            this.falg=false;
            var url = this.Global.aport11 + "/tkdqtj/getTkdqDataList";
            this.$api.post(url, p,
              r => {
                if (r.success) {
            ...
            this.falg=true;
            this.$api.post(this.Global.aport11+'/tkdqtj/getCount', p,
              r => {
                  this.tableData = r.data.resultList;
                  this.TotalResult = r.data.totalResult;
